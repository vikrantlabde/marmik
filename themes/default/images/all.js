...
var Todos = {
  reorder: function() {
    $(document.body).addClassName("reordering");
    this.makeDraggable();
  },
  
  stopReordering: function() {
    $(document.body).removeClassName("reordering");
  },
  
  makeDraggable: function(id) {
    Sortable.create($('incomplete_items'), { handle: 'di_' + id });
    ...
    if (!this.observer_installed) {
      ...
      Draggables.addObserver({
        ...
        onEnd: function(name, draggable, event) {
          ...
          new Ajax.Request(element.readAttribute("reorder_path"), { parameters: { position: position }});
        }
      })
    }
  },
...
Element.addMethods({
  slideIntoView: function(element, options) {
    ...
    var viewTop = document.body.scrollTop, viewHeight = window.innerHeight, viewBottom = viewTop + viewHeight;
  ...
  transition: function(element, change, options) {
    ...
    var wrapper = new Element("div"), clone = element.cloneWithoutIDs();
    element.insert({ before: wrapper });
    
    var offset = element.positionedOffset();    
    var originalHeight = element.getHeight();
    var width = element.getWidth() - parseInt(element.getStyle("paddingLeft")) - parseInt(element.getStyle("paddingRight"));
    wrapper.setStyle({ overflow: "hidden", position: "relative", height: originalHeight + "px" });
    clone.setStyle({ position: "absolute", top: 0, left: 0, width: width + "px", zIndex: 1000 });
    ...
    wrapper.insert({ before: outerWrapper });
    outerWrapper.setStyle({ overflow: "hidden", height: originalHeight + "px"});
    outerWrapper.appendChild(wrapper);
    
    var sizingEffect = height > originalHeight ? Effect.BlindDown : Effect.BlindUp;
    var scaleFrom    = height > originalHeight ? originalHeight / height * 100 : 100;
    var scaleTo      = height > originalHeight ? 100 : height / originalHeight * 100;
    var wrapHeight   = height > originalHeight ? height : originalHeight;
    
    wrapper.setStyle({ height: wrapHeight + "px" });
    clone.setStyle({ height: wrapHeight + "px" });

    var effects = [
      new Effect.Fade(clone, { sync: true }),
      new sizingEffect(wrapper, { sync: true, scaleFrom: scaleFrom, scaleTo: scaleTo }),
      new sizingEffect(clone, { sync: true, scaleFrom: scaleFrom, scaleTo: scaleTo })
    ];
    ...
    new Effect.Parallel(effects, {
      ...
      afterUpdate: function() {
        if (outerWrapper) {
          outerWrapper.insert({ before: wrapper });
          outerWrapper.remove();
          outerWrapper = false;
        }
      },
  ...
  transitionTo: function(element, className, options) {
    element = $(element);
    return element.transition(function() {
      element.addClassName(className);
    }, options);
  },
  
  transitionFrom: function(element, className, options) {
    element = $(element);
    return element.transition(function() {
...
Element.addMethods({
  upwards: function(element, selector) {
    element = $(element);
    if (element.match(selector)) return element;
...
var ResourceObserver = {
  start: function() {
    ...
    this.observer = this.onClick.bind(this);
    Event.observe(document, "click", this.observer);
  },
  
  stop: function() {
    if (!this.observer) return;
    Event.stopObserving(document, "click", this.observer);
    delete this.observer;
  },
  
  onClick: function(event) {
    var element = Event.element(event);
    var resourceElement = element.upwards(".resource");
    
    if (resourceElement && !element.match("input")) {
      var targetElement = element.upwards(".target");
      
      if (targetElement) {
        resourceElement = $(targetElement.readAttribute("resource") || resourceElement);
        var methodAndParams, instance = resourceElement.getResource();
        
        if (methodAndParams = targetElement.readAttribute("method")) {
          var method = (methodAndParams.match(/^([^\s]+)\s*/) || [0, ""])[1];
          var params = ("{" + ((methodAndParams.match(/\s(.+)$/) || [0, ""])[1]) + "}").evalJSON();
          params.target = targetElement;
          instance[method].call(instance, params);
...
Object.extend(Resource, {
  submitForm: function(form) {
    form = $(form);
    new Resource(form.up(".resource")).submit({ form: form });
    return false;
  }
});
Object.extend(Resource.prototype, {
  ...
  getClassName: function() {
    return ((this.element.id || "").match(/^(\w+)_\d+$/) || [])[1] || "resource";
  },
  
  getGroupName: function() {
    return this.element.readAttribute("group");
  },
  
  getStateName: function() {
    return this.element.readAttribute("state") || "show";
  },
  
  getStateElement: function() {
    return this.element.down(".state." + this.getStateName());
  },
  ...
  activate: function(options) {
    if (options.state != this.getStateName()) {
      this.deactivateResourcesInGroup();
      this.setActiveStateName(options.state);
      var field = this.getStateElement().down("input.autofocus");
      if (field) field.activate.bind(field).defer();
    } else {
      this.deactivate();
    }
  },
  ...
  deactivate: function(options) {
    this.unsetActiveStateName();
  },
  
  setActiveStateName: function(newStateName) {
    this.unsetActiveStateName();
    this.element.writeAttribute("state", newStateName);
    var element = this.getStateElement();
    if (element) {
      this.setClassNamesForState(newStateName);
    } else {
      this.element.writeAttribute("state", null);
    }
  },
  
  unsetActiveStateName: function() {
    this.unsetClassNamesForState();
    this.element.writeAttribute("state", null);
  },

  setClassNamesForState: function(stateName) {
    this.getStateElement().addClassName("active");
    this.element.addClassName("active");
    this.element.addClassName("in_" + stateName);

    var resourceClassName = this.getClassName();
    if (resourceClassName)
      document.body.addClassName("in_" + resourceClassName + "_" + stateName);
  },
  
  unsetClassNamesForState: function() {
    var stateName = this.getStateName();
    this.getStateElement().removeClassName("active");
    this.element.removeClassName("active");
    this.element.removeClassName("in_" + stateName);

    var resourceClassName = this.getClassName();
    if (resourceClassName)
      document.body.removeClassName("in_" + resourceClassName + "_" + stateName);
  },

  deactivateResourcesInGroup: function() {
    var groupName = this.getGroupName();
    if (groupName) {
      $$(".resource.active[group='" + groupName + "']").each(function(element) {
        if (element != this.element)
          new Resource(element).deactivate();
      }.bind(this));
    }
  },
  ...
  highlight: function() {
    this.element.visualEffect("highlight", {
      startcolor: "#ffff00",
      endcolor: (this.element.getStyle("backgroundColor") || "").parseColor(), 
      afterFinish: function() {
  ...
  remove: function() {
    ...
    this.element.visualEffect("fade", { 
      duration: 0.3,
      afterFinish: this.element.remove.bind(this.element)
    });
  },
  
  submit: function(options) {
    var form = options.form;
    if (form = form || options.target.up("form")) {
      this.setBusy();
      form.request({ onComplete: this.unsetBusy.bind(this) });
    }
  },
  ...
  request: function(method, options) {
    ...
    options = Object.extend(options, {
      method: method,
      onComplete: (options.onComplete || Prototype.K).wrap(function() {
    ...
    this.setBusy();
    return new Ajax.Request(this.getURL(), options);
  },
  
  update: function(options) {
    this.request("put", options);
  },
